...
public class PoolPainter extends Painter {
	...
	public static void paint( Level level, Room room ) {
		...
		for (int i=0; i < NPIRANHAS; i++) {
			OffspringOfVthyarilops offspringOfVthyarilops = new OffspringOfVthyarilops();
			do {
				offspringOfVthyarilops.pos = level.pointToCell(room.random());
			} while (level.map[offspringOfVthyarilops.pos] != Terrain.WATER|| level.findMob( offspringOfVthyarilops.pos ) != null);
			level.mobs.add(offspringOfVthyarilops);
		}
	}
