...
public class MiGoTounge extends Group {
	...
	public MiGoTounge(int from, int to, Callback callback){
		this(DungeonTilemap.tileCenterToWorld(from),
				DungeonTilemap.tileCenterToWorld(to),
				callback);
	}

	public MiGoTounge(PointF from, PointF to, Callback callback){
		super();
		...
		for (int i = 0; i < chains.length; i++){
			chains[i] = new Image(Effects.get(Effects.Type.MIGOTOUNGE));
