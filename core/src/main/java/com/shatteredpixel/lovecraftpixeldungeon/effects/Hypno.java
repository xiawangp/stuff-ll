...
public class Hypno extends Group {
	...
	public Hypno(int from, int to, Callback callback, float lifetime){
		this(Arrays.asList(new Arc(from, to)), callback, lifetime);
	}

	public Hypno(PointF from, int to, Callback callback, float lifetime){
		this(Arrays.asList(new Arc(from, to)), callback, lifetime);
	}

	public Hypno(int from, PointF to, Callback callback, float lifetime){
		this(Arrays.asList(new Arc(from, to)), callback, lifetime);
	}

	public Hypno(PointF from, PointF to, Callback callback, float lifetime){
		this(Arrays.asList(new Arc(from, to)), callback, lifetime);
	}

	public Hypno(List<Arc> arcs, Callback callback, float lifetime) {
	...
	public static class Arc extends Group {
		...
		public Arc(PointF from, PointF to){
			...
			arc1 = new Image(Effects.get(Effects.Type.HYPNO));
			arc1.x = start.x - arc1.origin.x;
			arc1.y = start.y - arc1.origin.y;
			arc1.origin.set( 0, arc1.height()/2 );
			add( arc1 );

			arc2 = new Image(Effects.get(Effects.Type.HYPNO));
