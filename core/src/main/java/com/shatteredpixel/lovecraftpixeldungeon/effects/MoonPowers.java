...
public class MoonPowers extends Group {
	...
	public MoonPowers(int from, int to, Callback callback, float lifetime){
		this(Arrays.asList(new Arc(from, to)), callback, lifetime);
	}

	public MoonPowers(PointF from, int to, Callback callback, float lifetime){
		this(Arrays.asList(new Arc(from, to)), callback, lifetime);
	}

	public MoonPowers(int from, PointF to, Callback callback, float lifetime){
		this(Arrays.asList(new Arc(from, to)), callback, lifetime);
	}

	public MoonPowers(PointF from, PointF to, Callback callback, float lifetime){
		this(Arrays.asList(new Arc(from, to)), callback, lifetime);
	}

	public MoonPowers(List<Arc> arcs, Callback callback, float lifetime) {
	...
	public static class Arc extends Group {
		...
		public Arc(PointF from, PointF to){
			...
			arc1 = new Image(Effects.get(Effects.Type.MOONFURY));
			arc1.x = start.x - arc1.origin.x;
			arc1.y = start.y - arc1.origin.y;
			arc1.origin.set( 0, arc1.height()/2 );
			add( arc1 );

			arc2 = new Image(Effects.get(Effects.Type.MOONFURY));
