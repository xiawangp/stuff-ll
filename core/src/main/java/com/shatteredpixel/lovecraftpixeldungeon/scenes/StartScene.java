...
import com.shatteredpixel.lovecraftpixeldungeon.windows.WndChallenges;
import com.shatteredpixel.lovecraftpixeldungeon.windows.WndClass;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		curClass = null;
		updateClass( HeroClass.values()[LovecraftPixelDungeon.lastClass()] );
	...
	private void startNewGame() {
		...
		if (LovecraftPixelDungeon.intro()) {
			LovecraftPixelDungeon.intro( false );
	...
	private class ChallengeButton extends Button {
		...
		protected void createChildren() {
			...
			image = Icons.get( LovecraftPixelDungeon.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF );
			add( image );
		}
		...
		protected void onClick() {
			if (Badges.isUnlocked( Badges.Badge.VICTORY )) {
				StartScene.this.add(new WndChallenges(LovecraftPixelDungeon.challenges(), true) {
					public void onBackPressed() {
						super.onBackPressed();
						image.copy( Icons.get( LovecraftPixelDungeon.challenges() > 0 ?
