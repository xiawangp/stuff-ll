...
import com.shatteredpixel.lovecraftpixeldungeon.ui.CheckBox;
import com.shatteredpixel.lovecraftpixeldungeon.ui.OptionSlider;
		...
		public DisplayTab() {
			...
			OptionSlider scale = new OptionSlider(Messages.get(this, "scale"),
				...
				protected void onChange() {
					if (getSelectedValue() != LovecraftPixelDungeon.scale()) {
						LovecraftPixelDungeon.scale(getSelectedValue());
						LovecraftPixelDungeon.switchNoFade((Class<? extends PixelScene>) LovecraftPixelDungeon.scene().getClass(), new Game.SceneChangeCallback() {
							@Override
			...
			CheckBox chkSaver = new CheckBox( Messages.get(this, "saver") ) {
				...
				protected void onClick() {
					...
					if (checked()) {
						checked(!checked());
						LovecraftPixelDungeon.scene().add(new WndOptions(
							...
							protected void onSelect(int index) {
								if (index == 0) {
									checked(!checked());
									LovecraftPixelDungeon.powerSaver(checked());
								}
							}
						});
					} else {
			...
			if (PixelScene.maxScreenZoom >= 2) {
				...
				chkSaver.checked(LovecraftPixelDungeon.powerSaver());
				add(chkSaver);
			}

			RedButton btnOrientation = new RedButton( LovecraftPixelDungeon.landscape() ?
					Messages.get(this, "portrait")
					: Messages.get(this, "landscape") ) {
				@Override
				protected void onClick() {
					LovecraftPixelDungeon.landscape(!LovecraftPixelDungeon.landscape());
				}
			};
			...
			OptionSlider brightness = new OptionSlider(Messages.get(this, "brightness"),
				...
				@Override
				protected void onChange() {
					LovecraftPixelDungeon.brightness(getSelectedValue());
				}
			};
			brightness.setSelectedValue(LovecraftPixelDungeon.brightness());
			...
			OptionSlider tileGrid = new OptionSlider(Messages.get(this, "visual_grid"),
				...
				@Override
				protected void onChange() {
					LovecraftPixelDungeon.visualGrid(getSelectedValue());
				}
			};
			tileGrid.setSelectedValue(LovecraftPixelDungeon.visualGrid());
	...
	private class UITab extends Group {
		...
		public UITab(){
			...
			RedButton btnSplit = new RedButton(Messages.get(this, "split")){
				@Override
				protected void onClick() {
					LovecraftPixelDungeon.toolbarMode(Toolbar.Mode.SPLIT.name());
			...
			RedButton btnGrouped = new RedButton(Messages.get(this, "group")){
				@Override
				protected void onClick() {
					LovecraftPixelDungeon.toolbarMode(Toolbar.Mode.GROUP.name());
					Toolbar.updateLayout();
				}
			};
			...
			RedButton btnCentered = new RedButton(Messages.get(this, "center")){
				@Override
				protected void onClick() {
					LovecraftPixelDungeon.toolbarMode(Toolbar.Mode.CENTER.name());
					Toolbar.updateLayout();
				}
			};
			...
			CheckBox chkFlipToolbar = new CheckBox(Messages.get(this, "flip_toolbar")){
				...
				protected void onClick() {
					super.onClick();
					LovecraftPixelDungeon.flipToolbar(checked());
					Toolbar.updateLayout();
				}
			};
			chkFlipToolbar.setRect(0, btnGrouped.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			chkFlipToolbar.checked(LovecraftPixelDungeon.flipToolbar());
			add(chkFlipToolbar);
			...
			final CheckBox chkFlipTags = new CheckBox(Messages.get(this, "flip_indicators")){
				...
				protected void onClick() {
					super.onClick();
					LovecraftPixelDungeon.flipTags(checked());
					GameScene.layoutTags();
				}
			};
			chkFlipTags.setRect(0, chkFlipToolbar.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			chkFlipTags.checked(LovecraftPixelDungeon.flipTags());
			add(chkFlipTags);

			OptionSlider slots = new OptionSlider(Messages.get(this, "quickslots"), "0", "4", 0, 4) {
				@Override
				protected void onChange() {
					LovecraftPixelDungeon.quickSlots(getSelectedValue());
					Toolbar.updateLayout();
				}
			};
			slots.setSelectedValue(LovecraftPixelDungeon.quickSlots());
			...
			CheckBox chkImmersive = new CheckBox( Messages.get(this, "soft_keys") ) {
				...
				protected void onClick() {
					super.onClick();
					LovecraftPixelDungeon.immerse(checked());
				}
			};
			chkImmersive.setRect( 0, slots.bottom() + GAP_SML, WIDTH, BTN_HEIGHT );
			chkImmersive.checked(LovecraftPixelDungeon.immersed());
			...
			CheckBox chkFont = new CheckBox(Messages.get(this, "system_font")){
				...
				protected void onClick() {
					...
					LovecraftPixelDungeon.switchNoFade((Class<? extends PixelScene>) LovecraftPixelDungeon.scene().getClass(), new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							LovecraftPixelDungeon.classicFont(!checked());
						}
			...
			chkFont.checked(!LovecraftPixelDungeon.classicFont());
			add(chkFont);
		}
		...
		public AudioTab() {
			OptionSlider musicVol = new OptionSlider(Messages.get(this, "music_vol"), "0", "10", 0, 10) {
				...
				protected void onChange() {
					...
					LovecraftPixelDungeon.musicVol(getSelectedValue());
				}
			};
			musicVol.setSelectedValue(LovecraftPixelDungeon.musicVol());
			...
			CheckBox musicMute = new CheckBox(Messages.get(this, "music_mute")){
				...
				protected void onClick() {
					super.onClick();
					LovecraftPixelDungeon.music(!checked());
				}
			};
			musicMute.setRect(0, musicVol.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			musicMute.checked(!LovecraftPixelDungeon.music());
			add(musicMute);
			...
			OptionSlider SFXVol = new OptionSlider(Messages.get(this, "sfx_vol"), "0", "10", 0, 10) {
				...
				protected void onChange() {
					...
					LovecraftPixelDungeon.SFXVol(getSelectedValue());
				}
			};
			SFXVol.setSelectedValue(LovecraftPixelDungeon.SFXVol());
			...
			CheckBox btnSound = new CheckBox( Messages.get(this, "sfx_mute") ) {
				...
				protected void onClick() {
					super.onClick();
					LovecraftPixelDungeon.soundFx(!checked());
					Sample.INSTANCE.play( Assets.SND_CLICK );
				}
			};
			btnSound.setRect(0, SFXVol.bottom() + GAP_TINY, WIDTH, BTN_HEIGHT);
			btnSound.checked(!LovecraftPixelDungeon.soundFx());
			add( btnSound );
