...
public class WndTextInput extends Window {
	...
	public WndTextInput(final String title, final String initialValue, final int maxLength,
		...
		final int width;
		if (LovecraftPixelDungeon.landscape() && multiLine){
			width = W_LAND_MULTI; //more editing space for landscape users
		} else {
			width = WIDTH;
		}

		LovecraftPixelDungeon.instance.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				...
				textInput = new EditText(LovecraftPixelDungeon.instance);
				...
				LovecraftPixelDungeon.instance.addContentView(textInput, layout);
			}
		});
	}
		...
		if (textInput != null){
			LovecraftPixelDungeon.instance.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					InputMethodManager imm = (InputMethodManager) LovecraftPixelDungeon
					...
					LovecraftPixelDungeon.updateImmersiveMode();
