...
public class WndLangs extends Window {
	...
	public WndLangs(){
		...
		for (int i = 0; i < langs.size(); i++){
			...
			RedButton btn = new RedButton(Messages.titleCase(langs.get(i).nativeName())){
				...
				protected void onClick() {
					...
					LovecraftPixelDungeon.switchNoFade(TitleScene.class, new Game.SceneChangeCallback() {
						@Override
						public void beforeCreate() {
							LovecraftPixelDungeon.language(langs.get(langIndex));
			...
			if (LovecraftPixelDungeon.landscape() && i % 2 == 1){
		...
		resize(LovecraftPixelDungeon.landscape() ? WIDTH_L : WIDTH_P, y);
