...
public class WndBag extends WndTabbed {
	...
	public WndBag( Bag bag, Listener listener, Mode mode, String title ) {
		...
		nCols = LovecraftPixelDungeon.landscape() ? COLS_L : COLS_P;
		...
		Bag[] bags = {
			...
			stuff.getItem( PotionBandolier.class ),
				...
				stuff.getItem( WeaponCase.class ),
	...
	private class BagTab extends Tab {
		...
		private Image icon() {
			...
			} else if (bag instanceof WeaponCase) {
				return Icons.get(Icons.WEAPONCASE);
				...
				if (item.name() == null) {
					...
					enable(
						...
						mode == Mode.SEED && (item instanceof Seed) ||
								mode == Mode.ANCIENTPOT && (item instanceof PotionOfFrost || item instanceof PotionOfLiquidFlame) ||
