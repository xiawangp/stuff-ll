...
import com.shatteredpixel.lovecraftpixeldungeon.sprites.WalkingShoogothSprite;
...
public class WalkingShoggoth extends Mob {

	{
		HP = HT = 50+Dungeon.depth;
		EXP = 10;
		defenseSkill = 12+Dungeon.depth;
		spriteClass = WalkingShoogothSprite.class;
	...
	public boolean act() {
		...
		if (Level.water[pos] && HP < HT) {
			...
			if (HP*2 == HT) {
				...
				((WalkingShoogothSprite)sprite).spray(false);
			}
			HP++;
		}
	...
	public static TwoShoggoth spawnNormalAt(int pos ) {
		if (Level.passable[pos] && Actor.findChar( pos ) == null) {
			...
			shoggoth.sprite.emitter().burst(WalkingShoogothSprite.GooParticle.FACTORY, 5 );
	...
	public int attackProc( Char enemy, int damage ) {
		for (int n : PathFinder.NEIGHBOURS8) {
			int cell = this.pos + n;
			if (Actor.findChar( cell ) instanceof DoubleShoggoth) {
	...
	protected boolean doAttack( Char enemy ) {
		if (pumpedUp == 1) {
			((WalkingShoogothSprite)sprite).pumpUp();
			PathFinder.buildDistanceMap( pos, BArray.not( Level.solid, null ), 2 );
			for (int i = 0; i < PathFinder.distance.length; i++) {
				if (PathFinder.distance[i] < Integer.MAX_VALUE)
			...
			if (visible) {
				if (pumpedUp >= 2) {
					((WalkingShoogothSprite) sprite).pumpAttack();
				}
				else
